...
d_list[4][1] = 10000
print('Ex 2 > ', (c_list))
print('Ex 2 > ', (d_list)) # 얕은 복사이기 때문에, 중첩된 객체에 대해서 원복 객체도 그대로 변경이 일어났다.(원본을 지키지 못함: 원본의 의미가 없다)
...
print('Ex 3 > ', id(f_list)) # 우선 서로다른 주소값 참조

print('Ex 2 > ', e_list)
print('Ex 2 > ', f_list) #f_list와 e_list는 다른 형태를 갖게 된다. 깊은 요소까지 복사했기 때문이다.

# 왜 늘 deepcopy를 사용하면 안될까 만능처럼 보이는데.
# => 우선 메모리 소요. 만약 1억개의 원소 지닌 리스트를 deepcopy한다면 메모리 소요가 심할 것
